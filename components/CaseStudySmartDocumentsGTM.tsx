    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-6 items-stretch h-full">
      ...
      <div className="flex flex-col gap-3 h-full">
        ...
        <div className="border border-green-200 rounded-xl bg-green-50 p-4 flex flex-col justify-between flex-1">
          ...
          <div aria-hidden="true" className="w-full bg-blue-600 text-white text-sm font-bold py-3 rounded-lg text-center">
...
export default function CaseStudySmartDocumentsGTM() {
  ...
  const handleTabChange = (index: number) => {
    ...
    if (seen >= total && !firedMilestones.current.has("full")) {
      ...
      trackCaseStudyProgress("smart-documents-gtm", "full");
    } else if (
      seen >= Math.ceil(total / 2) &&
      !firedMilestones.current.has("half")
    ) {
      firedMilestones.current.add("half");
      trackCaseStudyProgress("smart-documents-gtm", "half");
    }
  };
